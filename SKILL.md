...
```bash
conda run --no-capture-output -n pdf_read python "{AGENT_SKILL_py_PATH}" "{INSERT_PDF_PATH_HERE}"
```
...
- 输入路径 `{AGENT_SKILL_py_PATH}`  和 `{INSERT_PDF_PATH_HERE}`  两边的英文双引号**必须**有，这样才能保证文件的路径即使有空格或者其他特殊的符号，也能正常运行python脚本，并保证把**完整的、正确的字符串**传入到`pdf_2_md_main.py` 脚本
